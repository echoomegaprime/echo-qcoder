...
The Qwen model runtime itself is separately governed by the loopback-only deployment in [FORGE Qwen route](FORGE_QWEN_ROUTE.md). Raw Ollama remains isolated on `127.0.0.1:11438`; QCoder and SDK callers use the fail-closed gateway on port 11437 and the single stable 128K alias.
