...
catalog now covering three reviewed batches of 20 repositories each (60 total),
...
AI-red-team, reverse-engineering, mobile-instrumentation, or Prometheus
strike-team route. Batch 2 re-verified all 20 originally proposed candidates
...
red-teaming entries (Garak, PyRIT). Batch 3 adds reverse-engineering and
binary-analysis tooling (Ghidra, capa, FLOSS, YARA, jadx, Androguard, RetDec,
LIEF, Capstone, angr, syzkaller, Frida, and eight more) matching the tool
families named in `FLEET_ROLES/reverse-engineer.md`; it required extending
`policy.allowedLicenses` to recognize verified BSD-3-Clause, BSD-2-Clause, and
(Frida-only) wxWindows-3.1 license text that GitHub's classifier mis-tagged or
left unrecognized. It is an integration catalog, not a claim that all 60
binaries are installed; active, fuzzing, live-instrumentation, or
adversary-emulation tools remain Crucible-scoped.
